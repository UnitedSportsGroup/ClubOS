...
- **Refunds (per-item)**: Admin-initiated Stripe refunds at session granularity. From the registrations page, each paid registration shows a "Refund" (or "Refund More Sessions" when partially refunded) button. The dialog presents an item picker — one row per child × date × session — with checkbox, price (with proportional discount applied), and a "Refunded" badge that disables already-refunded items. Staff select sessions, see a running total, write an optional reason, and pass through a two-step confirm before the refund is sent to Stripe. Backend endpoint `POST /api/admin/registrations/:id/refund` accepts `{ itemIds: number[], reason? }` (also still supports `amountCents` for legacy/partial), computes per-item allocations using the Largest-Remainder method so `sum(itemAllocations) === refundAmount` exactly (no stranded cents), forces remaining-balance refund when the selected items are the last unrefunded ones, uses idempotency key `refund_reg_{id}_items_{sortedIds}`, and persists `registration_items.refunded_amount_cents` per row plus registration-level `refunded_at/amount/reason/by` and `stripe_refund_id/status`. Status becomes `partially_refunded` on first partial refund and `refunded` once the total is fully refunded. The expanded card shows a coloured refund summary block (Fully vs Partially Refunded), strike-through on refunded session rows with a per-item "Refunded $X" badge, and a Stripe refund-status badge with a manual "Refresh" button (`POST /api/admin/registrations/:id/refund/refresh-status`). Stripe webhook handlers for `charge.refunded`, `refund.updated`, `refund.failed`, and `charge.refund.updated` keep `stripe_refund_status` in sync. The admin list includes `confirmed`, `refunded`, and `partially_refunded` registrations (both globally and when filtered by camp).
